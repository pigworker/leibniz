...
outer :: ScreenState -> State -> IO Int
  ...
  inner ps@(p, s) st lc@(l, c@(cx, cy)) d = do
    ...
    case d' of
      LotsChanged -> do
        clearScreen
        resetCursor
        mapM_ putStr (layout (black, white) (cropLay cropBox ps' l))
      LineChanged -> do
        resetCursor
        down (cy - py)
        mapM_ putStr (layout (black, white) (cropLay cropBox ((px, cy), (sw, 1)) l))
      _ -> return ()
    ...
    case mc of
      ...
      Just Quit -> case st of (_, _, k, _) -> return k
...
myGC h = do
  m <- catch (hGetChar h >>= \ c -> return (Just c)) $ \ _ -> return Nothing
  case m of
    Nothing -> return ""
    Just c -> (c :) <$> myGC h
...
main = do 
  ...
  k' <- case drop k exs of e : es -> outer ((0, 0), s') (prog, (Root, e), k, es)
  endwin
